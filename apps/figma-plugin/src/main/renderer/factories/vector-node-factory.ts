...
export const vectorNodeFactory: DesignIrNodeFactory<DesignIrVectorNode> = {
  ...
  async create(node, context) {
    if (context.svgAdapter && node.inlineSvg) {
      try {
        context.reportProgress({ stage: "CREATING_VECTOR_NODES", completedNodes: 0, totalNodes: context.document.metrics.totalNodeCount, currentIrNodeId: node.id, message: "Creating inline SVG node." });
        const target = context.svgAdapter.createNodeFromSvg(node.inlineSvg);
        ...
        target.setPluginData("aio:vectorStatus", "INLINE_SVG_AVAILABLE");
        return { irNodeId: node.id, figmaNodeId: target.id, childContainer: false, placeholder: false };
      } catch {
        if (context.options.assetFailurePolicy === "FAIL_RENDER") throw new RendererError("RENDER_NODE_CREATE_FAILED", "Inline SVG could not be rendered.", node.id);
      }
    }
